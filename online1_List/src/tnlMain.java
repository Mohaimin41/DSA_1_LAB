...
//        ArrList<tnlStop> TNLstops = new ArrList<>(stops);
        ...
        System.out.println("tnl length: " + TNLstops.length());
    ...
    static void printTNLstops(list<tnlStop> tnlStopsList) {
        ...
        for (int i = 0; i < tnlStopsList.length(); i++) {
            tnlStopsList.moveToPos(i);

            System.out.print(i + " " + tnlStopsList.getValue().toString());
        ...
        if (current < tnlStopsList.length())
            tnlStopsList.moveToPos(current);
    }
}
